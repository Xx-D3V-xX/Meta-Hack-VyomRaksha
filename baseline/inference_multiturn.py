"""
VyomRaksha — baseline/inference_multiturn.py
...
You will receive a series of telemetry observations, one per turn.
...
Rules:
  ...
  - Consider your previous actions and their outcomes before deciding.
...
def _build_step_message(obs_dict: dict[str, Any], step: int) -> str:
...
def run_episode(env: VyomRakshaEnvironment, task_id: int, client: Any) -> list[dict]:
    """
    Run one full episode for the given task_id with a maintained conversation.
    ...
    while not obs.episode_done and step < MAX_STEPS_PER_EPISODE:
        ...
        user_msg = _build_step_message(obs_dict, step)
        ...
        # Append assistant reply to history before parsing (preserves exact text)
...
if __name__ == "__main__":
    ...
    print("\n=== VyomRaksha Multi-Turn Baseline Scores ===")
